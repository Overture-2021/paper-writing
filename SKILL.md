...
## Mandatory High-Level Writing Guideline

When revising ML paper prose, always apply the style discipline distilled from
`references/writing-guidelines.md`. Treat this as a top-level constraint before
section-specific advice, especially for logical progression and terminology definition:

1. Build an explicit logical progression: premise/time anchor -> mechanism -> evidence -> consequence.
2. Define every load-bearing term at first use, preferably with an inline gloss, before relying on it.
3. Keep terminology stable; once a quantity has notation, use the notation consistently.
4. Prefer concrete mechanism language over value-laden or anthropomorphic language.
5. State claims positively and precisely; avoid defensive "not merely/not only" framing unless contrast is essential.
6. Calibrate epistemic verbs: use "measures/shows" for direct observations and "estimates/suggests" for inferred quantities.
7. Use active methodological subjects for deliberate choices: "we define", "we estimate", "we compare".
8. Remove restatements and apology sentences; state scope once, plainly, then move on.
...
4. Maintain sentence-to-sentence flow with explicit cause, contrast, consequence, or refinement.
...
4. Keep terminology stable across the full paper; define key terms before reuse and use notation once introduced.
